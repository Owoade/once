...
import { InitializeTransaction, TransactionInitialized } from "paystack-sdk/dist/transaction/interface";
   ...
   async getProviderCheckout( providerKey: OnceProvider, id: string ){
    const transaction = await Transaction.findById( id );
     ...
     if( providerKey === "FLW" ){
        ...
        const flutterwaveCheckout = await flutterwave.initializeTransaction( flutterwavePayload as FlutterWaveTransactionInit )
     ...
     const paystackCheckout = await paystack.transaction.initialize( paystackPayload );
